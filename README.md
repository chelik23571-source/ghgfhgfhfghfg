...
Public gifts are read through Telegram's dedicated `payments.getUserStarGifts` API request, not from `users.getFullUser` fields. Install the required Telethon version from `pyproject.toml`; if the installed client does not expose this request, the diagnostic log reports that an upgrade is required.

`SCAN_REVISION` is set to `gifts-api-v2`, so the bot rechecks members previously marked as scanned by the older, incorrect full-profile-field check. Do not change it unless the detection method changes again.
