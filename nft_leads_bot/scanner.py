...
def count_collectible_gifts(gifts: Iterable[object]) -> int:
    """Count `StarGiftUnique` NFT gifts and exclude ordinary Star Gifts."""
    return sum(_is_collectible_gift(getattr(saved_gift, "gift", saved_gift)) for saved_gift in gifts)


def _is_collectible_gift(gift: object) -> bool:
    # Do not infer NFT status from attributes such as `slug` or `attributes`:
    # ordinary gifts can expose those fields too. Telegram's explicit TL type
    # is the only accepted proof that a gift is an upgraded collectible NFT.
    return type(gift).__name__ == "StarGiftUnique"
...
class ConsentScanner:
                ...
                try:
                    ...
                    async for member in self.client.iter_participants(entity):
                        ...
                        if not member.username:
                            skipped += 1
                            logger.debug("Skipping participant without username: chat_id=%s user_id=%s", chat_id, member.id)
                            continue
    ...
    async def _has_public_collectible(self, user: User) -> bool:
        ...
        # This count covers all profile gifts, including ordinary non-NFT gifts.
        # It is diagnostic only and must not qualify the profile as an NFT holder.
        gifts_count = int(getattr(details, "stargifts_count", 0) or 0)
        fields = ("gifts", "profile_gifts", "star_gifts", "premium_gifts")
        signals = {field: bool(getattr(details, field, None)) for field in fields}
        signals["profile_gifts_count"] = gifts_count > 0
        collectible_nft_count = await self._public_collectible_nft_count(user)
        signals["collectible_nft_gifts"] = collectible_nft_count > 0
        logger.debug(
            "NFT check: user_id=%s username=%r full_user_type=%s profile_gifts_count=%d collectible_nft_count=%d signals=%s",
            user.id,
            user.username,
            type(details).__name__ if details is not None else None,
            gifts_count,
            collectible_nft_count,
            signals,
        )
        return signals["collectible_nft_gifts"]

    async def _public_collectible_nft_count(self, user: User) -> int:
        """Return only public collectible/NFT gifts from Telegram's gift endpoint.
        ...
        gifts = gifts or ()
        count = count_collectible_gifts(gifts)
        logger.debug(
            "Public gifts loaded: user_id=%s response_type=%s gift_types=%s collectible_nft_count=%d",
            user.id,
            type(response).__name__,
            [type(getattr(item, "gift", item)).__name__ for item in gifts],
            count,
        )
        return count
